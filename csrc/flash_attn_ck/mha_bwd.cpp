...
void dump_dense_bwd_debug(const fmha_bwd_traits& traits,
                          const fmha_bwd_args& args,
                          const at::Tensor& dq_accum,
                          bool is_dropout,
                          bool deterministic) {
    if(!ck_bwd_debug_enabled()) {
        return;
    }

    const auto nsplits = dq_accum.size(0);
    const auto seqlen_q = dq_accum.size(2);
    const auto nhead_q = dq_accum.size(3);
    const auto hdim_q = dq_accum.size(4);
    const auto runner_split_stride_dq_acc = seqlen_q * hdim_q;
    const auto runner_nhead_stride_dq_acc =
        static_cast<long long>(runner_split_stride_dq_acc) * nsplits;
    const auto runner_batch_stride_dq_acc = nhead_q * runner_nhead_stride_dq_acc;

    std::cerr << "=== DEBUG fmha_bwd dense call ===\n";
    std::cerr << "traits:"
    ...
    std::cerr << "runtime:"
              ...
              << "\n";
    std::cerr << "mask:"
              << " window_left=" << args.window_size_left
              << " window_right=" << args.window_size_right
              << " mask_type=" << args.mask_type
              << "\n";
    std::cerr << "dq_accum tensor:"
              << " shape=[" << dq_accum.size(0) << ", " << dq_accum.size(1) << ", "
              << dq_accum.size(2) << ", " << dq_accum.size(3) << ", " << dq_accum.size(4) << "]"
              << " stride=[" << dq_accum.stride(0) << ", " << dq_accum.stride(1) << ", "
              << dq_accum.stride(2) << ", " << dq_accum.stride(3) << ", " << dq_accum.stride(4) << "]"
              << "\n";
    ...
    std::cerr << "dq_acc runner(expected):"
              ...
              << " batch=" << runner_batch_stride_dq_acc
              << " stride=" << hdim_q
        ...
        std::optional<at::Tensor> &rng_state_)
{
    ...
    if (seqlen_q > 0) {
        ...
        dump_dense_bwd_debug(traits, args, dq_accum, is_dropout, deterministic);
